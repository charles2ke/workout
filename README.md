...
- **API settings** — each source has an "API settings" panel for your OAuth client ID, and both sources include optional token endpoint and API base overrides so you can point at your own CORS-enabled proxy when a provider blocks direct browser calls
...
2. Open **My Fitness → API settings** for that source and paste the OAuth client ID.
